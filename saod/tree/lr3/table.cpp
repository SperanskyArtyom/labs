...
int main()
{
    ...
    auto SDP1 = createSDP(A, 100);
    auto SDP2 = createSDPv2(A, 100);

    Vertex *trees[3] = {ISDP, SDP1, SDP2};
    for (int i = 0; i < 3; i++)
    {
    ...
    // 17, 18, 19
    text[17].setPosition(15, 20 + dy);
    text[17].setString(L"ИСДП");
    text[18].setPosition(15, 20 + 2 * dy);
    text[18].setString(L"СДП1");
    text[19].setPosition(15, 20 + 3 * dy);
    text[19].setString(L"СДП2");

    sf::RenderWindow window(sf::VideoMode(winSizeX, winSizeY), L"СДП");
